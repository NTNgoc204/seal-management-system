...
async function runTests() {
  ...
  const post = async (url, data, token) => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error(`POST ${url} failed with status ${res.status}:`, responseData);
  ...
  const get = async (url, token) => {
    ...
    try {
      ...
      const responseData = await res.json();
      if (!res.ok) {
        console.error(`GET ${url} failed with status ${res.status}:`, responseData);
        throw new Error(responseData.message || `HTTP ${res.status}`);
      }
      return responseData;
    } catch (err) {
  ...
  console.log(`Admin registered: ${adminReg.user.fullName} (isSystemAdmin: ${adminReg.user.isSystemAdmin})`);

  // 3. Create Event
  console.log('\n[Step 3] Creating Event (Spring 2026)...');
  const eventResponse = await post('/events', {
  ...
  // 4. Create Track inside Event
  console.log('\n[Step 4] Creating Track (Web Application)...');
  const track = await post(`/events/${eventId}/tracks`, {
  ...
  // 5. Create Round inside Track
  console.log('\n[Step 5] Creating Vòng loại (Round 1)...');
  const round = await post(`/events/${eventId}/tracks/${trackId}/rounds`, {
  ...
  const rubric = await post('/rubrics', {
    eventId,
    trackId,
    roundId,
  ...
  const crit1 = await post(`/rubrics/${rubricId}/criteria`, {
    code: 'CODE',
    name: 'Chất lượng Mã nguồn & Kiến trúc',
    weight: 0.6,
    maxScore: 10,
    description: 'Đánh giá cấu trúc thư mục, Clean Code và các design pattern áp dụng.'
  }, adminToken);

  // Add Criterion 2: TEAM (weight 0.4)
  const crit2 = await post(`/rubrics/${rubricId}/criteria`, {
    code: 'TEAM',
    name: 'Tương tác Git & Đóng góp thành viên',
    weight: 0.4,
    maxScore: 10,
    description: 'Tần suất commit, sự phối hợp phân tách nhánh và đóng góp cân bằng giữa các thành viên.'
  }, adminToken);
  console.log('Criteria added successfully.');

  // Lock Rubric so judges can submit grades
  const lockedRubricResponse = await post(`/rubrics/${rubricId}/lock`, {}, adminToken);
  ...
  const teamReg = await post('/teams/register', {
    eventId,
    trackId,
    teamName: 'Dev Rangers',
    membersList: [
      { email: 'member1@seal.com', fullName: 'Tran Member One', githubUsername: 'member1-git', studentId: 'SE180002' },
      { email: 'member2@seal.com', fullName: 'Le Member Two', githubUsername: 'member2-git', studentId: 'SE180003' }
    ]
  }, leaderToken);
  const teamId = teamReg.teamId;
  console.log(`Team registered successfully with ID: ${teamId}. Status is: ${teamReg.status}`);
  ...
  for (const m of invitedMembers) {
    ...
    const res = await fetch(`http://localhost:5000/api/teams/confirm-invite?token=${token}`);
  ...
  if (teamDoc.status !== 'confirmed') {
    throw new Error(`Team status should be confirmed, but got ${teamDoc.status}`);
  }

  const repoDoc = await GithubRepository.findOne({ teamId });
  console.log(`GitHub Repo Auto-created: ${repoDoc.repoName} (URL: ${repoDoc.repoUrl})`);

  // 12. Submit project topic details (Leader)
  console.log('\n[Step 12] Submitting team project topic...');
  const topicSubmit = await post('/teams/submit-topic', {
    teamId,
    title: 'SEAL Hackathon Core Platform',
    description: 'An enterprise management system for handling seals and contests.',
    documentationLink: 'https://github.com/seal-hackathon-2026/dev-rangers/blob/main/README.md'
  }, leaderToken);
  ...
  const syncResult = await post(`/analytics/repo/${repoDoc._id}/sync`, {}, leaderToken);
  ...
  const suggestions = await get(`/grades/suggestion?teamId=${teamId}&roundId=${roundId}&rubricId=${rubricId}`, adminToken);
  console.log('Gemini AI Suggestions received:');
  suggestions.forEach(s => {
    console.log(` - Criterion Code: ${s.criterionCode} | Suggested: ${s.suggestedScore}/10 | Comment: ${s.comment}`);
  });

  // 15. Submit Judge Grading Scores
  console.log('\n[Step 15] Submitting official Judge scores...');
  const submitGrades = await post('/grades/submit', {
    teamId,
    roundId,
    rubricId,
    overallComment: 'Dự án triển khai xuất sắc, code chuẩn chỉ và phân tách rõ ràng. Tần suất commit rất ấn tượng.',
    details: [
      { criterionId: crit1._id, scoreValue: 9.0, comment: 'Cấu trúc thư mục chuẩn REST, sạch sẽ.' },
      { criterionId: crit2._id, scoreValue: 8.5, comment: 'Đóng góp đồng đều giữa các thành viên, git log sạch.' }
    ]
  }, adminToken);
  console.log(`Official score submitted. Total weighted score: ${submitGrades.totalWeightedScore}/10`);

  // 16. Lock the Round to compute rankings & standings
  console.log('\n[Step 16] Locking Round 1 and finalizing rankings...');
  const lockRound = await post('/grades/lock-round', {
    eventId,
    roundId,
    trackId
  }, adminToken);
  console.log(`Round Locked successfully. Message: ${lockRound.message}`);
  console.log(`Rankings table calculated: Rank #1 is Team ID: ${lockRound.rankings[0].teamId} with Score: ${lockRound.rankings[0].averageScore} (isAdvanced: ${lockRound.rankings[0].isAdvanced})`);
